...
### Changes
- `Dockerfile`: split `server-deps` into `deps` (unpruned, shared) + `server-deps` (pruned, used by `runtime`, behavior unchanged); added a `dev` stage (`deps` + `client/node_modules`, self-building `client/dist` on first run if missing, `CMD ["npm", "run", "dev"]`).
