...
# ---------------------------------------------------------------------------
# Ensure optional stock_move columns (Postgres or shared SQLite)
# ---------------------------------------------------------------------------
def _ensure_stock_extras():
    ...
    with engine.begin() as conn:
        ...
        if dialect.startswith("sqlite"):
            rows = conn.exec_driver_sql("PRAGMA table_info(stock_move)").fetchall()
            have_cols = {str(r[1]) for r in rows}
            if not have_cols:
                return
            if "trans_date" not in have_cols:
                conn.exec_driver_sql(
                    "ALTER TABLE stock_move ADD COLUMN trans_date TEXT"
                )
            if "idem_key" not in have_cols:
                conn.exec_driver_sql(
                    "ALTER TABLE stock_move ADD COLUMN idem_key VARCHAR(64)"
                )
            return
